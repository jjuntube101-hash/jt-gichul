...
function BottomNavInner() {
  ...
  return (
    <nav aria-label="하단 메뉴" className="fixed bottom-0 left-0 right-0 z-50 px-4 pb-[env(safe-area-inset-bottom,0px)]">
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {isActive && (
                <span className="absolute -top-0.5 left-1/2 h-0.5 w-4 -translate-x-1/2 rounded-full bg-primary" aria-hidden="true" />
              )}
            </Link>
          );
        })}
